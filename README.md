...
| **Consistency Engine** | Every response passes 5 pre-flight checks: setting consistency, character voice matching, timeline integrity, plot-line tracking, and butterfly effect accumulation. |
...
Stage 6: Consistency Engine → 5 pre-response validations
...
- `pipeline-details.md` — Complete 8-stage documentation with edge cases and advanced techniques
- `prompt-templates.md` — Ready-to-use prompt templates for each stage
- `state-schema.md` — Structured state format specification
...
| **一致性引擎** | 每轮回复前运行 5 项校验：设定一致性、角色声音匹配、时间线完整性、情节线追踪、蝴蝶效应累积。 |
